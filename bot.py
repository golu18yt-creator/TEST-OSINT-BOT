...
def init_db():
    ...
    default_settings = [
        ...
        ),  # Default welcome gif/video link
        ("refer_reward", "3"),
    ]
...
def send_welcome_media(chat_id, text, reply_markup=None):
    ...
    # Check if media is Animation/GIF or Video (mp4)
    ...
    try:
        ...
        if not res_json.get("ok"):
            # Fallback to standard message if media link fails to load
            return send_message(chat_id, text, reply_markup=reply_markup)
        return res_json
    except Exception:
        return send_message(chat_id, text, reply_markup=reply_markup)
...
def handle_message(update):
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        current_welcome_text = get_setting("welcome_msg")
        current_media = get_setting("welcome_media")
        ...
        admin_info = (
            ...
            f"• <code>/setwelcomemedia <URL/GIF_LINK></code> - Set Welcome GIF/MP4\n"
    ...
    # Set Welcome Media (GIF / MP4 Video Link) Command
    elif text.startswith("/setwelcomemedia") and is_admin:
        new_media = text.replace("/setwelcomemedia", "").strip()
        if new_media:
            set_setting("welcome_media", new_media)
            send_message(chat_id, "✅ <b>Welcome media (GIF/MP4) successfully updated!</b>")
        else:
            send_message(chat_id, "⚠️ <b>Usage:</b> <code>/setwelcomemedia https://example.com/video.mp4</code>")
        return
    ...
    if text.startswith("/start"):
        ...
        # Send Welcome Media (GIF/MP4) along with text
