...
BOT_USERNAME = os.environ.get("BOT_USERNAME", "Tera_Osint_44bot")  # Without @
OWNER_USERNAME = os.environ.get(
    "OWNER_USERNAME", "kya_Karega_Jaanke"
)  # Without @

SUPER_ADMIN_ID = 8927308711  # Aapki Admin User ID
OWNER_PHONE_NUMBER = "8595900496"  # Yahan apna 10-digit mobile number daalein
...
user_states = {}
temp_lookup_data = {}  # JSON file download ke liye temporary store
BOT_ACTIVE = True
...
# Search logic me deduct karne se pehle role verify ki gayi hai:
if not is_admin:
...
def get_owner_profile_photo():
    ...
    try:
        ...
        if res.get("ok") and res["result"]["total_count"] > 0:
            return res["result"]["photos"][0][-1]["file_id"]
    except Exception as e:
        print(f"Error fetching owner photo: {e}")
    return None


def is_user_channel_member(user_id, channel_username):
...
def handle_message(update):
    ...
    if target_channel and not is_admin:
        if not is_user_channel_member(chat_id, target_channel):
            clean_channel_name = target_channel.replace("@", "")
            channel_link = f"https://t.me/{clean_channel_name}"

            join_msg = (
                f"📢 <b>Must Join Channel First!</b>\n\n"
                f"Bot use karne ke liye pehle hamare official channel ko join karein.\n\n"
                f"👉 <b>Channel:</b> @{clean_channel_name}"
            )
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        admin_info = (
            ...
            f"• Required Channel: <b>@{curr_chan.replace('@', '')}</b>\n\n"
            "<b>⚙️ Admin Settings & Commands:</b>\n\n"
            "1️⃣ <b>Add Channels (Force Sub):</b>\n"
            "• <code>/addchannel @YourChannel</code> - Force users to join\n"
            "• <code>/removechannel</code> - Disable Channel Lock\n\n"
            "2️⃣ <b>Set Welcome Message:</b>\n"
            "• <code>/setmsg <text></code> - Customize Welcome Text\n"
            "• <code>/setgif <url></code> - Set Welcome Anime GIF\n\n"
            "3️⃣ <b>Make Redeem Code:</b>\n"
            "• <code>/createcode <CODE> <CREDITS></code> (Auto 100 limit)\n"
            "• <code>/createcode <CODE> <CREDITS> <LIMIT></code>\n\n"
            "4️⃣ <b>Make New Admin:</b>\n"
            "• <code>/makeadmin <USER_ID></code> - Grant Admin Access\n"
            "• <code>/removeadmin <USER_ID></code> - Revoke Admin\n\n"
            "<b>User Controls:</b>\n"
            "• <code>/botoff</code> / <code>/boton</code> - Toggle Bot\n"
            "• <code>/ban <id></code> / <code>/unban <id></code> - Ban User\n"
            "• <code>/addcredits <id> <amt></code> - Direct Credits"
        )
        ...
        parts = text.split()
        if len(parts) == 2:
            chan_name = parts[1].replace("@", "").strip()
            set_setting("force_channel", chan_name)
            send_message(
                chat_id,
                f"✅ <b>Required Channel Added!</b>\nUsers must join <b>@{chan_name}</b>.",
            )
        else:
            send_message(
                chat_id,
                "⚠️ <b>Usage:</b> <code>/addchannel @YourChannelName</code>",
            )
        return
    ...
    if text.startswith("/start"):
        ...
        owner_photo_id = get_owner_profile_photo()
    ...
    # --- PHONE SEARCH & AUTO-DELETE LOGIC ---
        ...
        if text.isdigit() and len(text) == 10:
            ...
            # DEDUCT CREDITS ONLY IF NOT ADMIN/OWNER
            if not is_admin:
                update_credits(chat_id, -1)
