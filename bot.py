...
SUPER_ADMIN_ID = 8927308711  # Admin User ID
...
# Fallback Mock Data for testing/safety
MOCK_DATABASE = {
    "9876543210": {"name": "Test Owner", "status": "Protected"},
    "9123456780": {"name": "Demo User", "network": "Jio", "location": "Delhi"},
}
...
def process_redeem_code(user_id, code):
    ...
    cursor.execute(
        "SELECT credits, max_uses FROM redeem_codes WHERE UPPER(TRIM(code)) = ?",
        (clean_code,),
    )
    row = cursor.fetchone()
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM claimed_codes WHERE UPPER(TRIM(code)) = ?",
        (clean_code,),
    )
    ...
    cursor.execute(
        "SELECT 1 FROM claimed_codes WHERE user_id = ? AND UPPER(TRIM(code)) = ?",
        (user_id, clean_code),
    )
    if cursor.fetchone():
        conn.close()
...
def get_owner_profile_media():
    ...
    try:
        ...
        if res.get("ok") and res["result"]["total_count"] > 0:
            ...
            best_file_id = photo_sizes[-1]["file_id"]
            return best_file_id
    except Exception as e:
...
def fetch_phone_data(phone_number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Fallback to local mock data if external API fails
            if phone_number in MOCK_DATABASE:
                return MOCK_DATABASE[phone_number]
            return {
                "error": True,
                "message": f"Server error ({response.status_code}). API unavailable.",
            }
    except requests.exceptions.Timeout:
        # Fallback to local mock data on timeout
        if phone_number in MOCK_DATABASE:
            return MOCK_DATABASE[phone_number]
        return {
            "error": True,
            "message": "⏱️ API Timeout! The external server is not responding.",
        }
    except Exception as e:
        if phone_number in MOCK_DATABASE:
            return MOCK_DATABASE[phone_number]
        return {"error": True, "message": f"⚠️ Connection failed: {str(e)}"}
...
def handle_message(update):
    ...
    # --- FORCE SUB CHECK ---
    target_channel = get_setting("force_channel")
    channel_link = get_setting(
        "force_channel_link"
    ) or f"https://t.me/{target_channel.replace('@', '')}"

    if target_channel and not is_admin:
        if not is_user_channel_member(chat_id, target_channel):
            join_msg = (
                f"📢 <b>Must Join Channel First!</b>\n\n"
                f"Bot use karne ke liye pehle hamare official channel ko join karein.\n"
            )
            ...
            send_message(chat_id, join_msg, reply_markup=join_btn)
            return
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        curr_chan = get_setting("force_channel") or "None"
        curr_link = get_setting("force_channel_link") or "None"

        admin_info = (
            f"👑 <b>Admin Control Panel</b>\n"
            f"• Status: <b>{status_text}</b>\n"
            f"• Referral Reward: <b>{ref_rew} Credits</b>\n"
            f"• Channel ID/User: <b>{curr_chan}</b>\n"
            f"• Saved Link: <b>{curr_link}</b>\n\n"
            "<b>⚙️ Admin Settings & Commands:</b>\n\n"
            "1️⃣ <b>Add Channel (With Link):</b>\n"
            "• <code>/addchannel @Username https://t.me/+xyzLink</code>\n"
            "• <code>/removechannel</code> - Disable Channel Lock\n\n"
            "2️⃣ <b>Set Welcome Message & Media:</b>\n"
            "• <code>/setmsg <text></code> - Customize Welcome Text\n"
            "• <code>/setgif <GIF_or_VIDEO_URL></code> - Set Custom Media URL\n\n"
            "3️⃣ <b>Make Redeem Code:</b>\n"
            "• <code>/createcode <CODE> <CREDITS> <LIMIT></code>\n\n"
            "4️⃣ <b>User Controls:</b>\n"
        ...
        parts = text.split()
        if len(parts) >= 2:
            chan_username = parts[1].strip()

            if len(parts) >= 3:
                invite_link = parts[2].strip()
            else:
                if chan_username.startswith("http"):
                    invite_link = chan_username
                    chan_username = (
                        "@" + chan_username.split("/")[-1].replace("+", "")
                    )
                else:
                    clean_u = chan_username.replace("@", "")
                    invite_link = f"https://t.me/{clean_u}"

            set_setting("force_channel", chan_username)
            set_setting("force_channel_link", invite_link)

            send_message(
                chat_id,
                f"✅ <b>Required Channel Updated!</b>\n\n"
                f"• <b>Target:</b> <code>{chan_username}</code>\n"
            ...
            send_message(
                chat_id,
                "⚠️ <b>Usage Format:</b>\n"
                "<code>/addchannel @YourChannel https://t.me/YourInviteLink</code>",
            )
        return

    elif text == "/removechannel" and is_admin:
        set_setting("force_channel", "")
        set_setting("force_channel_link", "")
        send_message(chat_id, "✅ <b>Channel Requirement Removed!</b>")
        return
        ...
        if len(parts) >= 3 and parts[2].isdigit():
            ...
            send_message(
                ...
                f"• <b>Code:</b> <code>{code_str.upper()}</code>\n"
        ...
        send_message(
            chat_id,
            "✅ <b>Welcome GIF/Video Updated!</b>"
            if gif_url
            else "✅ Custom GIF/Video Removed!",
        )
        return
    ...
    if text.startswith("/start"):
        ...
        owner_file_id = get_owner_profile_media()
        ...
        credits = (
            "∞ Unlimited" if is_admin else (user_data[1] if user_data else 0)
        )
    ...
    # --- PHONE SEARCH LOGIC (STUCK BUG FIXED) ---
    elif user_states.get(chat_id) == "awaiting_phone_number":
        # Always clear state to prevent bot hanging
        user_states[chat_id] = None
        ...
        if text.isdigit() and len(text) == 10:
            ...
            # Fetch API data with safe fallback
            ...
            if isinstance(api_data, dict) and api_data.get("error"):
                send_message(
                    chat_id,
                    f"❌ <b>Search Failed!</b>\n\n{api_data.get('message', 'API Server Error.')}",
                )
                return

            # Deduct credit only on successful response
            if not is_admin:
            ...
            info_msg = (
                f"📞 <b>Lookup Details for:</b> <code>{text}</code>\n\n"
                f"<pre>{formatted_json}</pre>\n\n"
                f"⚠️ <b>Privacy Warning:</b> <i>This message will be deleted in 30 seconds for security reasons!</i>"
            )
            ...
            send_message(chat_id, "❌ Send a valid 10-digit number.")
        return
