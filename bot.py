...
def check_user_joined(user_id):
    ...
    try:
        ...
        return True  # Bypass on network error to avoid trapping users
    return False
...
def fetch_tg_to_num_data(tg_input):
    query = tg_input.replace("@", "").strip()
    target_url = f"{EXTERNAL_TG_API_URL}?query={query}"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            phone = data.get("phone") or data.get("number") or data.get("phone_number") or ""
            phone = str(phone).replace("+", "").strip()

            if not phone:
                return {
                    "status": "Error",
                    "message": "No phone number linked to this Telegram account or profile is private."
                }

            if phone.startswith("91") and len(phone) == 12:
                return {
                    "status": "Success",
                    "telegram": f"@{query}" if not query.isdigit() else query,
                ...
                return {
                    "status": "Error",
                    "message": "🚫 <b>Access Restricted!</b>\nInternational Telegram numbers are not supported."
                }

        else:
            return {"status": "Error", "message": "Failed to fetch data from TG Database."}
    except Exception as e:
...
def handle_message(update):
    ...
    # --- FEATURE: TG TO NUMBER ---
        ...
        send_message(
            chat_id,
            f"👤 Send Telegram Username or User ID (e.g., <code>@username</code> or <code>1234567890</code>):\n<i>(Balance: {bal_display})</i>"
        )
        ...
        send_message(chat_id, f"🔍 <b>Fetching TG Phone Number for:</b> <code>{text}</code>...")
        ...
        if res.get("status") == "Success":
            result_msg = (
                ...
                f"• <b>Target:</b> {res['telegram']}\n"
                f"• <b>Phone:</b> <code>{res['phone_number']}</code>\n"
