...
def fetch_phone_data(phone_number):
    ...
    # Fixed URL Construction handling query parameters properly
    base_url = EXTERNAL_API_URL.strip()
    if "?" in base_url:
        target_url = f"{base_url}&number={clean_num}"
    else:
        target_url = f"{base_url}?number={clean_num}"
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                return {"status": "Error", "message": f"Raw Response: {response.text[:200]}"}
...
def handle_message(update):
    ...
    if text == "/botoff" and is_admin:
        BOT_ACTIVE = False
        send_message(chat_id, "🔴 <b>Bot is now OFF.</b> Saare users ke liye bot pause kar diya gaya hai.")
        return

    if text == "/boton" and is_admin:
        BOT_ACTIVE = True
        send_message(chat_id, "🟢 <b>Bot is now ON!</b> Bot normal state me aa chuka hai.")
        return
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        admin_info = (
            ...
            f"• <code>/botoff</code> / <code>/boton</code>\n"
            f"• <code>/setwelcomemsg <TEXT></code>\n"
            f"• <code>/setgif</code> / <code>/resetgif</code>\n"
            f"• <code>/addchannel <CHANNEL_ID> <CHANNEL_LINK></code>\n"
            f"• <code>/clearchannels</code>\n"
            f"• <code>/createcode <CODE> <CREDITS> <LIMIT></code>\n"
            f"• <code>/makeadmin <USER_ID></code>\n"
        ...
        if new_media.startswith("http"):
            ...
            send_message(chat_id, "✅ <b>Welcome media update ho gaya hai!</b> 🎉")
        else:
            send_message(chat_id, "❌ <b>Invalid Link!</b> Kripya ek valid URL bhejein.")
        user_states[chat_id] = None
        return

    elif text == "/resetgif" and is_admin:
        default_media = "https://media.giphy.com/media/3oKIPnAiaMCws8nOsE/giphy.gif"
        set_setting("welcome_media", default_media)
        send_message(chat_id, "🔄 <b>Welcome GIF/Video default par reset ho gaya hai!</b>")
        return

    elif text.startswith("/addchannel") and is_admin:
        parts = text.split(maxsplit=2)
        if len(parts) == 3 and parts[2].startswith("http"):
            add_force_channel(parts[1], parts[2])
        ...
        if len(parts) >= 3 and parts[2].isdigit():
            add_redeem_code(parts[1], int(parts[2]), int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 100)
            send_message(chat_id, f"✅ <b>Redeem Code Created!</b> <code>{parts[1].upper()}</code>")
        else:
        ...
        if len(parts) == 2 and parts[1].isdigit():
            make_user_admin(int(parts[1]))
            send_message(chat_id, f"✅ User <code>{parts[1]}</code> ko <b>Admin</b> bana diya gaya hai!")
            send_message(int(parts[1]), "🎉 <b>Congratulations!</b> You are now an Admin.")
        else:
        ...
        if not broadcast_msg:
            send_message(chat_id, "⚠️ <b>Usage:</b> <code>/broadcast Your message...</code>")
            return
    ...
    if text.startswith("/start"):
        ...
        send_welcome_media(chat_id, msg_template.format(first_name=first_name), reply_markup=dynamic_keyboard)
        return
        ...
        send_message(chat_id, "🎟️ <b>Send your Redeem Code below:</b>", reply_markup=dynamic_keyboard)
        ...
        send_message(chat_id, res_msg, reply_markup=dynamic_keyboard)
        user_states[chat_id] = None
        return

    # --- FEATURE: TG TO NUMBER BUTTON CLICK ---
    elif text == "👤 TG to Number":
        send_message(chat_id, "👤 <b>Send Target Telegram User ID:</b> (e.g., <code>123456789</code>)", reply_markup=dynamic_keyboard)
        user_states[chat_id] = "awaiting_tg_lookup"
        return

    # --- FEATURE: PHONE LOOKUP BUTTON CLICK ---
    elif text == "📱 Phone Lookup":
        send_message(chat_id, "📞 <b>Send Target Mobile Number:</b> (e.g. <code>9876543210</code>)", reply_markup=dynamic_keyboard)
        user_states[chat_id] = "awaiting_phone_number"
        return

    # --- HANDLE ACTIVE STATES FOR LOOKUPS ---
    current_state = user_states.get(chat_id)

    if current_state == "awaiting_tg_lookup":
    ...
    # Default fallback if user types anything else randomly
    send_message(chat_id, "⚠️ Kripya niche diye gaye menu options ka use karein ya /start dabayein.", reply_markup=dynamic_keyboard)
...
def main():
    ...
    print("Bot starting continuous polling loop with fixed lookups...")
    offset = None
