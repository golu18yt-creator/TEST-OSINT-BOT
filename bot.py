...
# ---------------------------------------------------------
# Configurations (Apni details yahan change karein)
# ---------------------------------------------------------
BOT_TOKEN = os.getenv("BOT_TOKEN", "8687879528:AAHOavqELOL_IVbcdN3-EX00II5EsnoYZLU")
ADMIN_IDS = [int(i) for i in os.getenv("ADMIN_IDS", "8927308711").split(",") if i.strip()]
OWNER_PHONE_NUMBER = os.getenv("OWNER_PHONE_NUMBER", "8595900496").strip()

# Supabase Lookup API Endpoint
EXTERNAL_API_BASE_URL = os.getenv("EXTERNAL_API_URL", "https://nmdllpezcocquamhgpmb.supabase.co/functions/v1/lookup")
...
# ---------------------------------------------------------
# Compatible API Fetch Function
# ---------------------------------------------------------
def fetch_phone_data(phone_number: str):
    try:
        url = EXTERNAL_API_BASE_URL.split("?")[0]
        ...
        if response.status_code != 200:
            return {"success": False, "error": f"API HTTP Error: Status {response.status_code}"}

        try:
            data = response.json()
        except Exception:
            return {"success": False, "error": "Invalid response format received from API."}

        # Handling API Response Structures
        if isinstance(data, dict):
            if data.get("success") is False:
                err_detail = data.get("error") or data.get("details") or "API Returned failure."
                return {"success": False, "error": str(err_detail)}

            if "result" in data and isinstance(data["result"], dict):
                res = data["result"]
                if res.get("success") is False:
                    return {"success": False, "error": res.get("details", "Lookup failed.")}
...
async def add_credits_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn.commit()
        await update.message.reply_text(f"✅ Added `{amount}` credits to User ID `{target_id}`.", parse_mode="Markdown")
    except Exception:
...
async def create_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn.commit()
        await update.message.reply_text(f"✅ Code `{code}` created with `{credits_val}` credits for `{uses}` uses!", parse_mode="Markdown")
    except Exception:
