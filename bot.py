...
def get_owner_profile_media():
    """Owner ki latest Profile Photo/Video Telegram API se fetch karta hai"""
...
def handle_message(update):
    ...
    channel_link = get_setting("force_channel_link") or f"https://t.me/{target_channel.replace('@', '')}"
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        if len(parts) >= 2:
            ...
            if len(parts) >= 3:
                ...
                if chan_username.startswith("http"):
                    ...
                    chan_username = "@" + chan_username.split("/")[-1].replace("+", "")
                else:
        ...
        send_message(
            chat_id, "✅ <b>Welcome GIF/Video Updated!</b>" if gif_url else "✅ Custom GIF/Video Removed!"
        )
        return
    ...
    if text.startswith("/start"):
        ...
        sent = False
        if gif_url:
            res = send_animation(chat_id, gif_url, caption=welcome_text, reply_markup=reply_keyboard)
            if res and res.get("ok"):
                sent = True

        if not sent and owner_file_id:
            # First try sending as photo (works for both photo and static frames of video DP)
            res = send_photo(chat_id, owner_file_id, caption=welcome_text, reply_markup=reply_keyboard)
            if res and res.get("ok"):
                sent = True
            else:
                # If photo fails, try sending as animation (for video DP)
                res = send_animation(chat_id, owner_file_id, caption=welcome_text, reply_markup=reply_keyboard)
                if res and res.get("ok"):
                    sent = True
