...
# 👇 AAPKI VERCEL TG TO NUMBER API LINK
...
def init_db():
    ...
    default_settings = [
        ...
        ("welcome_gif", ""),
        ("refer_reward", "3"),
        ("force_channel", ""),       # Format: @yourchannel
        ("force_channel_link", ""),  # Format: https://t.me/yourchannel
    ]
...
# ---------------------------------------------------------
# TELEGRAM API HELPERS & FORCE JOIN CHECK
# ---------------------------------------------------------
...
def send_force_join_msg(chat_id):
    channel_link = get_setting("force_channel_link") or f"https://t.me/{get_setting('force_channel').replace('@', '')}"
    markup = {
        "inline_keyboard": [
            [{"text": "📢 Join Channel", "url": channel_link}],
            [{"text": "✅ Joined / Verify", "callback_data": "check_join"}]
        ]
    }
    send_message(
        chat_id,
...
def fetch_tg_to_num_data(tg_input):
    """
    Vercel TG to Number API Lookup Function
    API Endpoint: https://tg2num-botadminshere.vercel.app/?id={value}
    """
    clean_input = tg_input.replace("@", "").strip()

    # Vercel API check: Only supports Numeric User IDs
    if not clean_input.isdigit():
        return {
            "status": "Invalid",
            "message": "❌ <b>Username Not Supported!</b>\n\nYe API <code>@username</code> support nahi karti.\nKripya user ki <b>Numeric Telegram ID</b> (e.g., <code>123456789</code>) bhein.\n\n💡 <i>Tip: Numeric ID nikalne ke liye user ka koi message @userinfobot par forward karein.</i>"
        }

    # Format URL dynamically with parameter
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            phone = None
            if isinstance(data, dict):
                phone = data.get("phone") or data.get("number") or data.get("result") or data.get("mobile")
            ...
            if not phone:
                return {
                    "status": "Error",
                    "message": f"No record found for Telegram ID <code>{clean_input}</code> in database."
                }
            ...
            return {"status": "Error", "message": f"Vercel API Error ({response.status_code}). Try again later."}
    except Exception as e:
...
def handle_callback_query(callback_query):
    ...
    if data == "check_join":
        if check_user_joined(chat_id):
            answer_callback_query(query_id, "✅ Verification Success! Click /start")
            send_message(chat_id, "🎉 <b>Verified!</b> Send /start to access the main menu.")
        else:
            answer_callback_query(query_id, "❌ You haven't joined yet!")
...
def handle_message(update):
    ...
    if not is_admin and not check_user_joined(chat_id):
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        f_channel = get_setting("force_channel") or "None"

        admin_info = (
            f"👑 <b>Admin Control Panel</b>\n"
            f"• Status: <b>{status_text}</b>\n"
            f"• Referral Reward: <b>{ref_rew} Credits</b>\n"
            f"• Force Channel: <b>{f_channel}</b>\n\n"
            "<b>⚙️ Admin Commands:</b>\n\n"
            "1️⃣ <b>Force Join Controls:</b>\n"
            "• <code>/setchannel @yourchannel</code> - Set Channel\n"
            "• <code>/setchlink https://t.me/yourchannel</code> - Custom Link\n"
            "• <code>/delchannel</code> - Remove Force Join\n\n"
            "2️⃣ <b>Message & Code:</b>\n"
            "• <code>/setmsg <text></code> | <code>/setgif <url></code>\n"
    ...
    elif text.startswith("/setchannel") and is_admin:
        ch = text.replace("/setchannel", "").strip()
        if ch.startswith("@"):
            set_setting("force_channel", ch)
            send_message(chat_id, f"✅ <b>Force Channel set to:</b> {ch}\n<i>Make sure bot is ADMIN in that channel!</i>")
        else:
            send_message(chat_id, "⚠️ <b>Usage:</b> <code>/setchannel @yourchannel</code>")
        return

    elif text.startswith("/setchlink") and is_admin:
        link = text.replace("/setchlink", "").strip()
        if link.startswith("http"):
            set_setting("force_channel_link", link)
            send_message(chat_id, f"✅ <b>Custom Channel Link set:</b> {link}")
        else:
            send_message(chat_id, "⚠️ <b>Usage:</b> <code>/setchlink https://t.me/yourchannel</code>")
        return

    elif text == "/delchannel" and is_admin:
        set_setting("force_channel", "")
        set_setting("force_channel_link", "")
        send_message(chat_id, "✅ <b>Force Join disabled!</b>")
        return
        ...
        if len(parts) == 2 and parts[1].isdigit():
            ...
            update_credits(target_id, 999999)
            send_message(
                chat_id,
                f"👑 <b>New Admin Added!</b> User ID <code>{target_id}</code> now has full access & unlimited credits.",
            )
        else:
            send_message(
    ...
    # --- FEATURE: TG TO NUMBER (VERCEL API) ---
    elif text == "👤 TG to Number":
        user_data = get_user(chat_id)
        credits = user_data[1] if user_data else 0

        if not is_admin and credits < 1:
            send_message(chat_id, "❌ <b>Out of Credits!</b> Earn credits to use Telegram Lookup.")
            return

        bal_display = "∞ Unlimited" if is_admin else credits
        send_message(
            chat_id,
            f"👤 <b>Send Telegram Numeric User ID:</b> (e.g., <code>123456789</code>)\n"
            f"⚠️ <i>Note: This API only supports Numeric IDs (Not @username).</i>\n\n"
            f"<i>(Balance: {bal_display})</i>"
        )
        ...
        send_message(chat_id, f"🔍 <b>Fetching TG Phone Number for ID:</b> <code>{text}</code>...")
        ...
        if res.get("status") == "Success":
            ...
            result_msg = (
                f"🎯 <b>Telegram Lookup Success!</b>\n\n"
                f"• <b>Target ID:</b> <code>{res['telegram_id']}</code>\n"
                f"• <b>Phone Number:</b> <code>{res['phone_number']}</code>\n\n"
                f"⚠️ <i>Auto-deleting in 30 seconds for privacy.</i>"
            )
            send_auto_delete_message(chat_id, result_msg, delay=30)
        else:
            send_message(chat_id, f"{res.get('message')}")

        user_states[chat_id] = None
        return

    # --- FEATURE: PHONE LOOKUP ---
