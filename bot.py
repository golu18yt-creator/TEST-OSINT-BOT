...
def init_db():
    ...
    # Force clear forced channel lock from previous database instances
    cursor.execute("UPDATE settings SET value = '' WHERE key IN ('force_channel', 'force_channel_link')")
...
def clear_force_channel():
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE settings SET value = '' WHERE key = 'force_channel'")
    cursor.execute("UPDATE settings SET value = '' WHERE key = 'force_channel_link'")
    conn.commit()
    conn.close()
...
def fetch_phone_data(phone_number):
    ...
    target_url = f"{base_endpoint}?number={phone_number}"
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                # Clean external watermark/credits if present
...
def handle_message(update):
    ...
    channel_link = get_setting("force_channel_link") or (
        f"https://t.me/{target_channel.replace('@', '')}" if target_channel else ""
    )

    if target_channel and target_channel.strip() != "" and not is_admin:
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        clear_force_channel()
        send_message(chat_id, "✅ <b>Channel Requirement Removed Permanently!</b>")
        return
    ...
    # --- DIRECT OR STATE PHONE SEARCH LOGIC ---
    elif (user_states.get(chat_id) == "awaiting_phone_number") or (text.isdigit() and len(text) == 10):
        user_data = get_user(chat_id)
        credits = user_data[1] if user_data else 0
