...
def make_user_admin(target_user_id):
    ...
    if exists:
        ...
        # Agar user ne bot start nahi kiya par aapne admin bana diya toh entry create kar do
        cursor.execute(
...
def register_user(user_id, username, referrer_id=None):
    ...
    if existing:
        # Agar pehle se hai par super admin ya banaya gaya admin hai toh status maintain rakho
...
def fetch_tg_to_num_data(tg_input):
    ...
    if not clean_input.isdigit():
        return {
            "status": "Invalid",
            "message": "❌ <b>Invalid ID!</b> Kripya sirf <b>Numeric Telegram ID</b> (e.g., <code>123456789</code>) bhein.",
        }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not phone:
                return {
                    "status": "Error",
                    "message": f"❌ Target ID <code>{clean_input}</code> ka record nahi mila.",
                }
        ...
        return {"status": "Error", "message": "⚠️ API Server Error!"}
    except Exception as e:
...
def handle_message(update):
    ...
    if text == "⚡ Admin Panel" and is_admin:
        ...
        admin_info = (
            ...
            "• <code>/clearchannels</code> - Remove all channels\n"
            "• <code>/createcode <CODE> <CREDITS> <LIMIT></code>\n"
            "• <code>/makeadmin <USER_ID></code> - Promote user to Admin\n"
            "• <code>/broadcast <MESSAGE></code> - Send DM to all users"
        )
    ...
    # --- MAKE ADMIN FEATURE FIXED ---
        ...
        if len(parts) == 2 and parts[1].isdigit():
            ...
            send_message(chat_id, f"✅ User <code>{target_id}</code> ko successfully <b>Admin</b> bana diya gaya hai aur unlimited credits de diye gaye hain!")
            
            # Target user ko uske DM par notification
            admin_notify_msg = (
                "🎉 <b>CONGRATULATIONS!</b> 🎉\n\n"
                "👑 You are now an <b>Admin</b> of this bot!\n"
                "🛡️ <b>Role:</b> Administrator\n"
                "💳 <b>Credits:</b> ∞ Unlimited\n\n"
                "<i>Ab aap bot ke admin features use kar sakte hain. /start bhejiye menu open karne ke liye.</i>"
            )
    ...
    if text.startswith("/start"):
        ...
        send_message(chat_id, welcome_text, reply_markup=dynamic_keyboard)
        return
    ...
    # --- FEATURE: TG TO NUMBER ---
        ...
        send_message(chat_id, f"🔍 <b>Fetching target details for ID:</b> <code>{text}</code>...")
    ...
    # --- FEATURE: PHONE LOOKUP & OWNER PROTECTION ---
        ...
        send_message(chat_id, f"🔍 Searching details for: <code>{text}</code>...")
        if not is_admin:
            update_credits(chat_id, -1)
