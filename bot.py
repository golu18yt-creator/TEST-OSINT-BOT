...
def get_owner_profile_media():
    """Owner ki latest Profile Photo ya Profile Video dynamic fetch karta hai"""
    ...
    try:
        ...
        if res.get("ok") and res["result"]["total_count"] > 0:
            ...
            # Telegram profile photo/video array format check
            best_quality = photo_sizes[-1]
            file_id = best_quality["file_id"]

            # File info check (check if video or photo)
            file_info_url = f"{TELEGRAM_API_BASE}/getFile"
            file_res = requests.post(file_info_url, json={"file_id": file_id}, timeout=10).json()
            if file_res.get("ok"):
...
def handle_message(update):
    ...
    if text.startswith("/start"):
        ...
        # Dynamic Owner Media Fetch (Photo or Video DP)
